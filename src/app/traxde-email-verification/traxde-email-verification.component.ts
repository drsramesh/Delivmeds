...
@Component({
  selector: 'app-traxde-email-verification',
  templateUrl: './traxde-email-verification.component.html',
  styleUrls: ['./traxde-email-verification.component.css']
})
export class TraxdeEmailVerificationComponent implements OnInit {
    ...
    //   this.preLoader.open();
      this.activatedRoute.queryParams.subscribe(params => {
        ...
        if(this.confirmKey !== undefined){
          ...
          this.auth.traxdeEmail(confirmToken).subscribe(
            (res:any) =>{ console.log(res)
              ...
              if(res.code ==1){
                ...
                this.messageService.add({severity: 'success', summary: 'Success', detail: 'Pharmacy verified successfully.'});
              
              }else if(res.statusCode ==401) {
                this.showButton = 401;
                this.msgs = [];
                this.messageService.add({severity: 'error', summary: 'Error', detail: 'Pharmacy not verified.'});
                // this.router.navigate(['/login']);
